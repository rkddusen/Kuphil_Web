...
function getPath(btn){
    ...
    if (btn.value=="수의과대학 (정문)"){
        ...
        Path.id='kuphil_room';
        Pathway= '<div><img src="./image/introduce/map_front.jpg" alt="수의대에서 동방 오는 길"></div>';
        Pathway+='</div>';
        ...
        Path.setAttribute('class', 'path');
        Path.id='kuphil_room';
        Pathway= '<div><img src="./image/introduce/map_library.jpg" alt="도서관 (중문)에서 동방 오는 길"></div>';
        Pathway+='</div>';
        Path.innerHTML=Pathway;
        let pathway=document.getElementById('pathway');
        pathway.appendChild(Path);
        ...
        Path.setAttribute('class', 'path');
        Path.id='kuphil_room';
        Pathway= '<div><img src="./image/introduce/map_back.jpg" alt="경영대학 (후문)에서 동방 오는 길"></div>';
        Pathway+='</div>';
        Path.innerHTML=Pathway;
        let pathway=document.getElementById('pathway');
        pathway.appendChild(Path);
